#!/bin/bash
# 安装/更新 opencode（GitHub Release 固定版本）及其配置（~/.config/opencode/opencode.json）
# 固定版本写在 OPENCODE_VERSION；升级前用 tools/latest-version.sh opencode 查上游
# provider 全部用官方注册表（models.dev），不做自定义 provider 配置；密钥由 opencode-auth.sh 单独安装
...
if [[ "$REMOVE" == "1" ]]; then
    confirm_remove "opencode（CLI + 配置）" || exit 0
...
if [[ -x "$OPENCODE_BIN" ]]; then
    ...
    if [[ "$UPDATE" != "1" ]]; then
        echo "opencode 已安装: $OPENCODE_BIN (${installed_version:-unknown})"
    elif [[ "$installed_version" == "$OPENCODE_VERSION" ]]; then
